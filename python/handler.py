...
def db_perf(event, context):
    start_time = time.time()
    db_session = db_connection.get_db_session()
    print("--- %s start - connection established seconds ---" % round(time.time() - start_time, 5))
    # sql_query = f"""
    # CREATE TABLE entity (
    ...
    print("--- %s start - result fetched seconds ---" % round(time.time() - start_time, 5))
    print("global entity", result)
    return {
        'statusCode': 200,
